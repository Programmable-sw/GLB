...
| 最小公共视角 | 跨 QP 消费从 P4 的 **528,709** 增至 P16 的 **2,126,559**；正确 collective CCT 口径下，P4/P16 分别变差 **1.3%/4.0%**，只有 P8 改善 **2.2%** | 共享能减少盲发，但同步传播冷却也会移除容量并引起新一轮路径竞争 | 没有证据表明直接共享必然改善 CCT |
...
- CCT/p99 FCT：共享状态进入闭环后的次级性能结果。由于 QP 通过 trigger 分批启动，collective CCT 定义为 `max(finish_us) - min(start_us)`，不能用最大单流 FCT 代替。

### 重复探索随并发增长

| P | MRC 重复发现 | MRC-shared 重复发现 | shared 相对 MRC | 跨 QP 消费 | shared/MRC CCT | CCT 改善 seed |
|---:|---:|---:|---:|---:|---:|---:|
| 4 | 293,079 | 285,465 | -2.60% | 528,709 | **1.0132** | 1/3 |
| 8 | 534,037 | 517,686 | -3.06% | 993,221 | **0.9776** | 1/3 |
| 16 | 1,041,988 | 1,055,535 | +1.30% | 2,126,559 | **1.0397** | 1/3 |
...
- P4：CCT 增加 1.32%，只有 1/3 seed 改善；pooled p99 降 1.47%；
- P8：CCT 降 2.24%，但仍只有 1/3 seed 改善；pooled p99 降 1.00%；
- P16：CCT增加 3.97%，只有 1/3 seed 改善；pooled p99 只降 0.10%，逐流 FCT 几何均值增加 1.97%。

逐 seed 的正确 CCT 比值如下：

| P | seed 13 | seed 29 | seed 47 | 三 seed 几何均值 |
|---:|---:|---:|---:|---:|
| 4 | 0.9305 | 1.0599 | 1.0547 | **1.0132** |
| 8 | 1.0552 | 0.8605 | 1.0290 | **0.9776** |
| 16 | 0.9590 | 1.1135 | 1.0523 | **1.0397** |

逐流 FCT 分布进一步说明，这不是单调的“共享越多、每条流越快”：

| P | shared/MRC 逐流 FCT 几何均值 | FCT 中位数变化 | p95 变化 | p99 变化 | shared/MRC CCT | 净效果 |
|---:|---:|---:|---:|---:|---:|---|
| 4 | 0.9936 | -0.60% | -4.13% | -1.47% | **1.0132** | 大部分逐流分位数略好，但 collective 最后完成时间略差 |
| 8 | **0.9794** | -2.86% | -3.40% | -1.00% | **0.9776** | pooled 指标总体改善，但 CCT 只有一个 seed 改善，方向不稳定 |
| 16 | **1.0197** | **+4.16%** | -0.24% | -0.10% | **1.0397** | 中位流和 collective CCT 都变差，p99 基本不变 |

因此，如果问题是“跨 QP 共享是否一定改善 CCT”，本实验答案明确是否定的。三个并发档位都只有 1/3 seed 的 CCT 改善；P8 的三-seed 几何均值略好，但由 seed 29 的较大改善主导，不能解释为稳定收益。P16 中共享既没有普遍降低逐流 FCT，也没有降低正确口径的 collective CCT。

P16 中 shared 的重复发现还比独立 MRC 多 1.30%。这说明简单公共状态存在另一条反馈链：

`一个 QP 发布冷却 → 同域 QP 同时避开该 EV → 替代路径竞争加重 → 更多反馈状态变化或重复发现`

这也不是严格意义上“所有 QP 同时冲向同一条路径”的羊群效应。MRC-shared 只同步哪些 EV 应被冷却，各 QP 的 EV permutation、轮转 cursor、窗口和可靠性状态仍然独立，所以替代路径选择不会完全同步。更准确的风险是 **avoidance herd**：多个 QP 同时缩小候选集合，虽然它们仍在剩余 EV 间错开轮转，却会共同提高剩余路径的负载。P16 的逐流 FCT 和 CCT 同时变差，与这种容量集中风险一致；它是机制一致性证据，不应单独表述为已经证明的逐包因果链。
...
实验 3 使用非对称 WebSearch 开放到达流量，不是 All-to-All collective：

- 128 个节点、8 条物理路径；
- 40% 和 80% offered load，持续到达窗口 10 ms，仿真截止 40 ms；
- seeds 为 13/29/47；
- 随机稀疏选择 4 条 ToR 上行，将其速率降为正常路径的 1/2；
- 不注入动态故障，不增加实验 2 的链路背景负载；
- 流大小使用已有 WebSearch 离散分布，从 6 KiB 到 30 MiB；
- 40% 负载每个 K 汇总 43,457 条流，80% 汇总 86,948 条流；
- K=2/4/8 始终复用同一 traffic SHA-256，只改变每个 QP 的活跃 EV 前缀；物理路径空间始终为 8。

流量中约 60% 是 6–133 KiB 短流，667 KiB、1.3 MiB 和 3.3 MiB 各约占 10%，6.7 MiB 约占 7%，20–30 MiB 合计约占 3%。因此它同时包含“大量短流”和“少量决定总体 p99 的长流”。
...
|---:|---|---:|---:|---|
| 40% | 6–133 KiB | **1.0173** | 1.0021 | 短流略微变差 |
| 40% | 667 KiB–1.3 MiB | 0.9891 | 0.9987 | 基本持平 |
| 40% | 3.3–30 MiB | **0.9276** | 0.9904 | K2→K4 明显改善，K4→K8 收益很弱 |
...
同一 QP 的初始 EV 顺序在 K2/K4/K8 中是同一个确定性 permutation 的前缀。对于只发送少量 packet 的短流，其自身早期选择不会因为更大的 K 自动获得学习收益。短流 FCT 的变化主要来自共同运行的中长流改变路径占用后产生的排队外部性；因此不能把短流变差解释为“该短流自己的反馈误导了选路”。
...
- 当前 8 路、WebSearch 场景中，如果目标是总体 p99，K=4 是状态利用率与多样性的较好折中；如果目标是短流 FCT，K=2 反而更好。这不是可直接外推的固定最优值。

本实验没有 collective barrier，flow 在 10 ms 窗口内随机到达，因此不报告 CCT。`max(finish)-min(start)` 在这里主要反映到达窗口和最后一条随机流，不是有业务含义的 collective completion time。若要判断 EV 数量对 CCT 的影响，需要在固定总字节、固定 collective 起点的 All-to-All 或训练通信中单独交叉 K=2/4/8。
...
1. **带陈旧度和置信度的公共视角**：保持共享键不变，只增加 TTL、多个 QP 观察 quorum 或持续性门槛，验证能否把当前不稳定、P16 退化的 CCT 结果转化为跨 seed 的稳定收益。
