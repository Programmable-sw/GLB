...
|---:|---:|---:|---:|---:|---:|---|
| 4 | 0.9936 | -0.60% | -4.13% | -1.47% | **1.1072** | 大部分分位数略好，但最慢完成流更差，三个 seed 的 CCT 全部退化 |
| 8 | **0.9794** | -2.86% | -3.40% | -1.00% | **0.9811** | 本实验中最均衡：普通流和尾部总体改善，但仍有一个 seed 的 CCT 退化 |
| 16 | **1.0197** | **+4.16%** | -0.24% | -0.10% | **0.9341** | 中位流变慢，极端 straggler 减少，最终 CCT 改善 |

因此，如果问题是“高并发重复探索是否让所有 QP 的 FCT 变差”，本实验答案是否定的。P16 中共享状态主要改善最慢完成的一组 QP，而不是普遍降低逐流 FCT；若目标是平均或典型 FCT，结果反而偏负，若目标是同步通信的 barrier/CCT，则结果偏正。

P16 中 shared 的重复发现还比独立 MRC 多 1.30%。这说明简单公共状态存在另一条反馈链：

`一个 QP 发布冷却 → 同域 QP 同时避开该 EV → 替代路径竞争加重 → 更多反馈状态变化或重复发现`

这也不是严格意义上“所有 QP 同时冲向同一条路径”的羊群效应。MRC-shared 只同步哪些 EV 应被冷却，各 QP 的 EV permutation、轮转 cursor、窗口和可靠性状态仍然独立，所以替代路径选择不会完全同步。更准确的风险是 **avoidance herd**：多个 QP 同时缩小候选集合，虽然它们仍在剩余 EV 间错开轮转，却会共同提高剩余路径的负载。P16 的逐流 FCT 变差而 CCT 改善，与这种“减少极端坏路发送、但增加剩余路径竞争”的分布重塑一致；它是机制一致性证据，不应单独表述为已经证明的逐包因果链。
