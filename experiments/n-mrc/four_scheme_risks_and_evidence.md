...
|---|---:|---:|---:|---:|
| avail | **0.00196** | 0.02715 | 0.04339 | 0.11660 |
| grade | 0.00428 | **0.03776（最高）** | 0.06093 | **0.16582（最高）** |
| netaware | 0.00677 | 0.01726 | 0.04189 | 0.10631 |
...
P16 时序实验中，avail 最终累计 CV 为四方案最低的 0.00196；但是窗口 CV 均值为 0.02715，是 n-mrc 的约 4.2 倍，也高于 netaware。
...
同时，grade 的窗口 CV 均值 0.03776、最大值 0.16582，均为四方案最高。证据链为：
...
所以 netaware 在持续坏路场景的收益确实来自网络快照和 GOOD 路径偏置，不是随机结果。
...
所以“三条更优候选”确实是保守门槛，但这种保守具有保护作用。当前证据还显示回退集中在 20–30 MiB 大流，短流中位数并未明显变差。
...
限制：时序曲线目前只有一个代表 seed，适合证明“累计 CV 不等于窗口 CV”和展示机制差异；它不用于声称窗口 CV 的跨 seed 置信区间。性能因果结论仍以三 seed 配对消融为准。
