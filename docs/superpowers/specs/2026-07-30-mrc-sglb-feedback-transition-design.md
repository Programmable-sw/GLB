...
3. MRC should recover relative to SGLB. The MRC/SGLB FCT ratio should fall
   toward one; the claim does not require MRC to beat SGLB.
...
- Start fixed-link background traffic at time zero on five of eight spine
  choices, leaving three clean choices. Foreground flows start after 100 us,
  when SGLB's 1 us local and 5 us downstream state periods have elapsed.
- Keep the background active for the complete measurement window so that a
  falling MRC/SGLB ratio cannot be caused by the hotspot ending.
...
The background rate may be selected by a small seed-13 pilot. A valid rate
must:

- complete every foreground flow in every scheme;
- make SGLB state discriminative rather than all-zero for nearly all route
  calls;
...
4. MRC/SGLB FCT falls toward one over that transition and is not driven by one
   seed.
5. MRC/RR improves over the same transition, providing feedback-ablation
